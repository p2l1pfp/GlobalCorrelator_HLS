...
void _lut_zbin_init(ap_uint<16> _table[512]) {
  _table[0] = 0;
  for (int i = 1; i <= 511; ++i) {
    _table[i] = (i / Z0_SCALE)*(NVTXBINS/16);
...
void _lut_zcbin_init(z0_t _table[NVTXBINS]) {
  ...
  for (int i = 0; i < NVTXBINS; ++i) {
    _table[i] = (16/NVTXBINS)*(Z0_SCALE*i+Z0_SCALE*0.5);
  }
}
...
void simple_vtx_hwopt(TkObj track[NALLTRACK], VtxObj *outvtx) { 
  ...
  outvtx->hwZ0  = outvtx->hwZ0 << NVTXPOW;
  outvtx->hwZ0 += 8;//Z0_SCALE*0.5
  //outvtx->hwZ0 = outvtx->hwZ0 << (NVTXPOW-4);
