...
void fwdlinpuppi_ref(const HadCaloObj caloin[NCALO], PFNeutralObj pfallne[NCALO], PFNeutralObj pfselne[NNEUTRALS], bool debug) {
    ...
    for (int in = 0; in < NCALO; ++in) {
        ...
        if (debug) printf("ref candidate %02d pt %7.2f  em %1d: alpha %+7.2f   x2a %+5d = %+7.3f  x2pt %+5d = %+7.3f   x2 %+5d = %+7.3f  --> weight %4d = %.4f  puppi pt %7.2f\n",
                   ...
                   pfallne[in].hwPtPuppi*LINPUPPI_ptLSB);
...
void fwdlinpuppi_flt(const HadCaloObj caloin[NCALO], PFNeutralObj pfallne[NCALO], PFNeutralObj pfselne[NNEUTRALS], bool debug) {
    ...
    const float f_ptZeroNe = LINPUPPI_ptZeroNe;
    const float f_ptZeroPh = LINPUPPI_ptZeroPh;
    const float f_alphaSlopeNe = LINPUPPI_alphaSlope;
    const float f_alphaSlopePh = LINPUPPI_alphaSlope;
    const float f_alphaZeroNe = LINPUPPI_alphaZero;
    const float f_alphaZeroPh = LINPUPPI_alphaZero;
    const float f_alphaCrop = LINPUPPI_alphaCrop;
    const float f_priorNe = LINPUPPI_priorNe;
    const float f_priorPh = LINPUPPI_priorPh;
    ...
    for (int in = 0; in < NCALO; ++in) {
        ...
        float x2pt    = ptSlope * (caloin[in].hwPt*LINPUPPI_ptLSB - ptZero);
        ...
        if (debug) printf("flt candidate %02d pt %7.2f  em %1d: alpha %+7.2f   x2a         %+7.3f  x2pt         %+7.3f   x2         %+7.3f  --> weight        %.4f  puppi pt %7.2f\n",
                   in, caloin[in].hwPt*LINPUPPI_ptLSB, int(caloin[in].hwIsEM), std::max(alpha,-99.99f), x2a, x2pt, x2, weight, pfallne[in].hwPtPuppi*LINPUPPI_ptLSB);
    }
