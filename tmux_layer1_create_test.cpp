...
int objDataLength[4] = {2*NTRACK, 2*(NPHOTON+NTRACK), 2*(NPHOTON+NSELCALO+NTRACK), 2*(NPHOTON+NSELCALO+NTRACK+NMU)};
int link_max[4] = {NLINKS_PER_PFCH, NLINKS_PER_PFCH+NLINKS_PER_PFPHO, NLINKS_PER_PFCH+NLINKS_PER_PFNH+NLINKS_PER_PFPHO, NLINKS_PER_PFCH+NLINKS_PER_PFNH+NLINKS_PER_PFPHO+NLINKS_PER_PFMU};
int link_min[4] = {0, NLINKS_PER_PFCH, NLINKS_PER_PFCH+NLINKS_PER_PFPHO, NLINKS_PER_PFCH+NLINKS_PER_PFPHO+NLINKS_PER_PFNH};
unsigned int theEtaRegion = 0;
unsigned int thePhiRegion = 0;

//unsigned int outputOrder[NREGIONS] = {0,2,4,6,8,10,12,14,16,15,17,1,3,5,7,9,11,13};//NPHI x NETA
//unsigned int outputOrder[NREGIONS] = {0,11,1,12,2,13,3,14,4,15,5,16,6,17,7,9,8,10};//NPHI x NETA
unsigned int outputOrder[NREGIONS] = {0,2,1,3,11,4,12,5,13,6,14,7,15,8,16,9,17,10};//NPHI x NETA
//mapping from Ryan:
//  eta, phi — 0,0 --- 1,8
...
int main() {
    ...
    //std::string datawords [NTEST*NREGIONS][mp7DataLength+1];
    //for (int ia = 0; ia < NTEST*NREGIONS; ia++){
    //    for (int ib = 0; ib < mp7DataLength+1; ib++){
    //        datawords[ia][ib] = "0000000000000000";
    //    }
    //}
    const int listLength = NTEST*NREGIONS;
    std::string datawords [listLength][NLINKS_APX_GEN0];
    for (int ia = 0; ia < listLength; ia++){
    ...
    for (int test = 0; test < NTEST; ++test) {
        ...
        if (!inputs.nextRegion_tmux(calo, emcalo, track, mu, hwZPV)) isData = false;
        ...
        HadCaloObj calo_temp[NREGIONS][NCALO]; EmCaloObj emcalo_temp[NREGIONS][NEMCALO]; TkObj track_temp[NREGIONS][NTRACK]; MuObj mu_temp[NREGIONS][NMU];
        ...
        int i_temp[NREGIONS] = {0};
        int ireg = 0;
        int ntracks[NREGIONS] = {0};
        int ncalos[NREGIONS] = {0};
        int nemcalos[NREGIONS] = {0};
        int nmus[NREGIONS] = {0};
        if (isData) {
            int Ntracks=0;
            int Ncalos=0;
        ...
        for (int ir = 0; ir < NREGIONS; ir++) {
            ...
            //    printf("data_in[%i] = %i \n", in, (unsigned int) data_in[in]);
            //}
                
            mp7wrapped_pfalgo3_full(data_in, data_out, hwZPV);
            PFChargedObj outch_ref[NTRACK];
            PFNeutralObj outpho_ref[NPHOTON];
            PFNeutralObj outne_ref[NSELCALO];
            PFChargedObj outmupf_ref[NMU];
            MP7_REF_FUNC(emcalo_temp[ir], calo_temp[ir], track_temp[ir], mu_temp[ir], outch_ref, outpho_ref, outne_ref, outmupf_ref);
            ...
            /*for (int id = 0; id < mp7DataLength; id++) {
                ...
                datawords[test*NREGIONS+ir][id] = stream1.str();
            }
            std::stringstream stream1;
            stream1 << "00000000";
            stream1 << std::uppercase << std::setfill('0') << std::setw(6) << std::hex << (((unsigned int)(hwZPV.range(9,0))) << 14) << "00";
            datawords[test*NREGIONS+ir][mp7DataLength] = stream1.str();
