...
if true; then #for debugging
	...
	echo
        ...
        sed "s/II=[0-9]*/II=${II}/g" run_hls_hgcpfalgo_w_puppi.tcl > run_hls_test.tcl
	...
	#vivado_hls -f run_hls_fullpfalgo_test.tcl
        rm -rf l1pfpuppi-hgc-resource-test
...
if true; then #for debugging
	...
	SRC_BUILD_DIR=$SCRIPT_DIR/build_hgc.tcl
	...
	sed -i s/XX_NINPUTS_XX/$((2*(${NTRACKS} + ${NCALOS}) + 4))/g $DEST_BUILD_DIR
